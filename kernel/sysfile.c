...
sys_open(void)
{
  ...
  if (!(omode & O_SYMLNK))
  {
    int rec_cnt = 16; // MAX ALLOWED link layers
    struct inode *next_f;
    while (rec_cnt && ip->type == T_SYMLINK)
    {
...
uint64 sys_symlink()
{
  ...
  if (argstr(0, tar_path, MAXPATH) < 0)
  {
    return -1;
  }
  ...
  begin_op();
  ip = create(path, T_SYMLINK, 0, 0); // create a file
  if (ip == 0)
  {
    end_op();
    return -1;
  ...
  int len = strlen(tar_path) + 1;
  if (writei(ip, 0, (uint64)tar_path, 0, len) != len)
  {
    iunlockput(ip);
    end_op();
    return -1;
