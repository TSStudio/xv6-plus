...
int kfork(void) {
    ...
    *(np->trapframe) = *(p->trapframe);
    ...
    np->sig_trapframe_valid = 0;
    memset(&np->sig_trapframe, 0, sizeof(np->sig_trapframe));
    for (i = 0; i < NSIG; i++)
        np->signal_handlers[i] = p->signal_handlers[i];
    np->signal_handlers_valid = p->signal_handlers_valid;
